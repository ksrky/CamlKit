...
let rec zonk_aexp : aexp -> aexp = function
  ...
  | AppAExp {fcn= fcn, fcn_ty; arg= arg, arg_ty} ->
      AppAExp {fcn= (zonk_aexp fcn, zonk_ty fcn_ty); arg= (zonk_aexp arg, zonk_ty arg_ty)}
  | LamAExp {vars; body= body, body_ty} -> LamAExp {vars; body= (zonk_aexp body, zonk_ty body_ty)}
  | OpAExp {left= left, left_ty; op; right= right, right_ty} ->
      OpAExp
        {left= (zonk_aexp left, zonk_ty left_ty); op; right= (zonk_aexp right, zonk_ty right_ty)}
  ...
  | LetAExp {bnds; body= body, body_ty} ->
      LetAExp {bnds= List.map zonk_abnd bnds; body= (zonk_aexp body, zonk_ty body_ty)}
  | LetrecAExp {bnds; body= body, body_ty} ->
      LetrecAExp {bnds= List.map zonk_abnd bnds; body= (zonk_aexp body, zonk_ty body_ty)}
