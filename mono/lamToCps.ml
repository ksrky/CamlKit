...
let rec l2k_ty : L.ty -> K.ty = function
  | IntTy -> IntTy
  | BoolTy -> BoolTy
  | FunTy (ty1, ty2) -> ContTy [l2k_ty ty1; l2k_cont ty2]
  | _ -> failwith "TODO"

and l2k_cont ty : K.ty = ContTy [l2k_ty ty]

let l2k_var (id, ty) : K.var = (id, l2k_ty ty)

let rec l2k_exp (exp : L.exp) (k : K.valty) : K.exp =
  match exp with
  | Const (Int _ as c) -> K.mk_app k (K.Const c, IntTy)
  | Const (Bool _ as c) -> K.mk_app k (K.Const c, BoolTy)
  | Var (id, ty) -> K.mk_app k (K.Var id, l2k_ty ty)
  | App {fcn= fcn, fcn_ty; arg= arg, arg_ty} ->
      let fcn_ty' = l2k_ty fcn_ty in
      let arg_ty' = l2k_ty arg_ty in
  ...
  | Lam {var; body= body, body_ty} ->
      let body_ty' = l2k_ty body_ty in
      let cont_id = Id.from_string "cont" in
      K.App
        { fcn= k
        ; args=
            [ ( K.mk_lams
                  [l2k_var var; (cont_id, K.ContTy [body_ty'])]
                  (l2k_exp body (Var cont_id, ContTy [body_ty']))
              , ContTy [l2k_ty (snd var); ContTy [body_ty']] ) ] }
  | Prim {left= left, left_ty; oper; right= right, right_ty} ->
      ...
      let left_ty' = l2k_ty left_ty in
      let right_ty' = l2k_ty right_ty in
  ...
  | If {cond= cond, cond_ty; then_= then_, _; else_= else_, _} ->
      let cond_ty' = l2k_ty cond_ty in
      let cond_id = Id.from_string "cond" in
      l2k_exp cond
        ( K.mk_lam (cond_id, cond_ty')
            (K.If
               { cond= (Var cond_id, cond_ty')
               ; then_= l2k_exp then_ k
               ; else_= l2k_exp else_ k } )
        , ContTy [cond_ty'] )
  | Let {var; bnd; body= body, _} ->
      l2k_exp bnd
        (K.mk_lam (l2k_var var) (l2k_exp body k), ContTy [l2k_ty (snd var)])
  | Tuple _ -> raise Utils.Unreachable
  | Proj _ -> raise Utils.Unreachable

and l2k_def ({var; param; body= body, body_ty} : L.def) : K.def =
  let body_ty' = l2k_ty body_ty in
  let cont_id = Id.from_string "c" in
  { var= l2k_var var
  ; params= [l2k_var param; (cont_id, K.ContTy [body_ty'])]
  ; body= l2k_exp body (Var cont_id, K.ContTy [body_ty']) }

let l2k_prog ((exp, ty) : L.prog) : K.prog =
  let exp_ty = l2k_ty ty in
