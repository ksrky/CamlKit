...
let%test _ = Compile.f (Builtin ("mul", [Int 2; Int 4])) = [LDC 4; LDC 2; MUL; STOP]

let%test _ =
  Compile.f (Lam ([x; y], Builtin ("add", [Var x; Var y])))
  = [LDF [LD (1, 2); LD (1, 1); ADD; RTN]; STOP]

let%test _ =
  Compile.f (Let (false, [x], [Int 1], Builtin ("printi", [Var x])))
